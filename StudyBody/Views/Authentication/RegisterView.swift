...
struct RegisterView: View {
        ...
        VStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Picker("Департамент", selection: $viewModel.department) {
                    ...
                    ForEach(UserDepartment.allCases) { dep in
                        Text(dep.title).tag(dep as UserDepartment?)
                    }
                }
                .pickerStyle(.menu)
    ...
    private func resendVerificationEmail() async {
        do {
            try await AuthService.shared.resendVerificationEmail(email: viewModel.email)
